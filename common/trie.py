...
class Trie (object):
    ...
    def add(self, word, value=1):
        curr_node = self.root
        for ch in word:
            curr_node.trans.setdefault(ch, Trie.TrieNode(ch))
...
g_character_encoder_dict = {}
def character_based_encoder_v3 (ch, range=('a', 'z')):
    global g_character_encoder_dict
    c = ord(ch)
    if c not in g_character_encoder_dict:
        i = len(g_character_encoder_dict) + 1
        g_character_encoder_dict[c] = i
    return g_character_encoder_dict[c]

def character_based_encoder_v2 (ch, range=('a', 'z')):
    ret = ord(ch) - ord(range[0]) + 1
    if ret <= 0: ret = ord(range[1]) + 1
    return ret

def character_based_encoder_v1 (ch, range=('a', 'z')):
    return ord(ch)

class DATrie (object):
    def __init__(self, chr_encoder=character_based_encoder_v1):
        self.root = 0
    ...
    def match_longest (self, word):
        ...
        return (0, ret_l, ret_v)

    def search_longest (self, word):
        max_match_len = 0
        max_match_start_pos = 0
        max_match_val = None
        i = 0
        while i < len(word):
            spos, l, val  = self.match_longest(word[i:])
            if max_match_len < l:
                max_match_len = l
                max_match_start_pos = i
                max_match_val = val
            i += 1
        return max_match_start_pos, max_match_len + max_match_start_pos, max_match_val
    ...
    def save (self, fname):
        ...
        using_32bits = int (log (l, 2)) > 15
        fmt_str = '%dl'%l if using_32bits else '%dh'%l

        f.write (struct.pack ('L', l))
        f.write (struct.pack ('H', using_32bits))
    ...
    def load (self, fname):
        ...
        using_32bits = struct.unpack ('H', f.read(2))[0]
...
def match_longest (trie, word):
    return trie.match_longest(word)
...
def test2(fname):
    ...
    datrie = DATrie()
    datrie.build(words)
    print datrie.match_longest(u'我是特种兵之国之利刃')
    print datrie.search_longest(u'我是特种兵之国之利刃')

def test1():
    datrie = DATrie()
    datrie.build(words=[u'吃饭还是房子', u'吃饭还是玩', u'中华人民共和国'], values=[1,2,3])
    print datrie.match_longest(u'我吃饭还是玩')
    print datrie.match_longest(u'吃饭还是玩')
    print datrie.match_longest(u'吃饭还是玩垃圾')
    print datrie.search_longest(u'我们吃饭还是玩垃圾')
    print datrie.search_longest(u'168331184')
